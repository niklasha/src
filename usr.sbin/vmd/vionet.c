	...
	for (;;) {
		...
		case VIODEV_MSG_IO_READ:
			...
			imsg_compose_event2(iev, IMSG_DEVOP_MSG, 0, 0, -1, &msg,
			    sizeof(msg), ev_base_main);
			/*
			 * Emit the ISR-read interrupt deassert on the SAME
			 * async channel the assert uses
			 ...
			 * device's IOAPIC line, eliminating an
			 * assert(async)/deassert(sync) cross-thread reorder
			 * that could clobber a coalesced level completion.
			 * vionet_deassert_pic_irq() must run on the main
			 * thread, which is exactly where handle_sync_io()
			 * executes.
			 */
			if (deassert)
...
vionet_assert_pic_irq(struct virtio_dev *dev)
{
	 ...
	 * reset (or the guest's ISR read) has already cleared, closing that
	 * reset-vs-raise race, while never dropping a live raise (isr writes
	 * by workers and the guest ISR-read clear are all serialized by
	 * `lock`, so isr != 0 here iff an interrupt is genuinely pending).
	 */
	pthread_rwlock_rdlock(&lock);
	if (dev->isr == 0) {
		pthread_rwlock_unlock(&lock);
