	...
	if (dir == VEI_DIR_OUT) {
		...
		case VIO1_PCI_DEVICE_STATUS:
			...
			if (dev->status == 0) {
				...
				/*
				 * A reset must lower the interrupt line.  After
				 * reset the guest will not read the ISR
				 * register, so without an explicit deassert the
				 * level line would stay asserted if a
				 * completion was in service -- a false
				 * interrupt source (the P_no_stuck_line
				 * obstruction).  Mirror vionet's reset
				 * deassert.  Only multi-process devices
    ...
    void *cookie, uint8_t sz)
{
	...
	if (msg.type == VIODEV_MSG_IO_WRITE) {
		...
		if (msg.type == VIODEV_MSG_IO_READ && msg.data_valid) {
			...
			*data = msg.data;
			/*
			 * Option A: the synchronous reply carries DATA ONLY and
			 * never mutates the device's IOAPIC line.  Every line
			 * op (assert AND the ISR-read deassert) is emitted by
			 * the device on its async KICK channel and applied by
			 * the VM process event-loop thread (handle_dev_msg),
			 * making that thread the sole, in-order mutator of the
			 * line.  This removes the assert(async)/deassert(sync)
			 * cross-thread reorder on the level line (CE-C): a
			 * coalesced completion can no longer be clobbered by a
			 * stale deassert running on the vcpu thread.
			 */
		} else {
