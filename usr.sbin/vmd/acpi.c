...
/*
 * Align x up to the next multiple of 4.  Used to keep each table in the
 * "etc/acpi/tables" blob 4-byte aligned.
 */
...
acpi_init(uint32_t ncpus)
{
	...
	e->u.add_pointer.offset = offsetof(struct acpi_rsdp, rsdt_addr);
	...
	strlcpy(e->u.add_pointer.src_file, ACPI_FILE_TABLES,
	    sizeof(e->u.add_pointer.src_file));
	e->u.add_pointer.offset = rsdt_off +
	    offsetof(struct acpi_rsdt, entry[0]);
	e->u.add_pointer.size = 4;
	...
	strlcpy(e->u.add_pointer.src_file, ACPI_FILE_TABLES,
	    sizeof(e->u.add_pointer.src_file));
	e->u.add_pointer.offset = rsdt_off +
	    offsetof(struct acpi_rsdt, entry[1]);
	e->u.add_pointer.size = 4;
	...
	strlcpy(e->u.add_pointer.src_file, ACPI_FILE_TABLES,
	    sizeof(e->u.add_pointer.src_file));
	e->u.add_pointer.offset = fadt_off + offsetof(struct acpi_fadt, dsdt);
	e->u.add_pointer.size = 4;
