    ...
    int child_disks[][VM_MAX_BASE_PER_DISK], int *child_taps)
{
	...
	acpi_pmtimer_init();
	/*
	 * ACPI PM I/O block: 0x600-0x60B (PM base).  Only the PM timer at +8
	 * returns meaningful data; other offsets return 0 on read and ignore
	 * writes.  NOTE: MUST stay below VM_PCI_IO_BAR_BASE (0x1000): the PCI
	 * I/O BAR window (0x1000-0xFFFF) is mapped to vcpu_exit_pci just
	 * below and would otherwise overwrite these entries, misrouting
	 * PM1/PM-timer accesses to the PCI dispatcher (guest ACPI reads
	 * garbage -> the "PM1 stuck" SCI spin).  This is the same PM-timer
	 * base the ACPI variant of this table generator uses for its FADT.
	 */
	...
	/*
	 * ELF boot: write MP table for guest SMP discovery.  IOAPIC's APIC ID
	 * must not collide with any LAPIC's APIC ID (LAPIC IDs are
	 * 0..ncpus-1).  If it does, Linux renumbers the colliding LAPIC, and
	 * subsequent IOAPIC RTE writes carry the renumbered (out-of-range)
	 * APIC ID, causing silent IRQ drops in ioapic_assert_to_lapic.  ncpus
	 * is safely above the LAPIC range.
	 */
	if (vmc->vmc_ncpus > 1) {
		/*
