...
extern int translate_gva(struct vm_exit *, uint64_t, uint64_t *, int);
...
mmio_find(uint64_t gpa)
{
	...
	for (i = 0; i < mmio_n_handlers; i++) {
		struct mmio_handler *h = &mmio_handlers[i];
...
static uint64_t op_mov(uint64_t a, uint64_t b) { return (b); }
...
mmio_op_for(enum x86_opcode_type op)
{
	...
	if (op >= sizeof(ops) / sizeof(ops[0]) || ops[op].compute == NULL)
		return (NULL);
	return (&ops[op]);
}
...
insn_decode(struct vm_exit *exit, struct x86_insn *insn)
{
	...
	if (insn->insn_opcode.op_type == OP_GROUP1 &&
		...
		switch (MODRM_REGOP(insn->insn_modrm)) {
		case 0: insn->insn_opcode.op_type = OP_ADD; break;
		case 1: insn->insn_opcode.op_type = OP_OR;  break;
		case 4: insn->insn_opcode.op_type = OP_AND; break;
		case 5: insn->insn_opcode.op_type = OP_SUB; break;
		case 6: insn->insn_opcode.op_type = OP_XOR; break;
		case 7: insn->insn_opcode.op_type = OP_CMP; break;
		default:
	...
	if (insn->insn_opcode.op_type == OP_BT_GROUP &&
	    insn->insn_modrm_valid) {
		switch (MODRM_REGOP(insn->insn_modrm)) {
		case 4: insn->insn_opcode.op_type = OP_BT;  break;
		case 5: insn->insn_opcode.op_type = OP_BTS; break;
		case 6: insn->insn_opcode.op_type = OP_BTR; break;
		case 7: insn->insn_opcode.op_type = OP_BTC; break;
		default:
...
emulate_generic(struct x86_insn *insn, struct vm_exit *exit)
{
	...
	if (op == OP_TEST) {
		uint8_t sub = MODRM_REGOP(insn->insn_modrm);
		if (sub != 0 && sub != 1) {
