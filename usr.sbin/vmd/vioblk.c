...
handle_sync_io(int fd, short event, void *arg)
{
	...
	for (;;) {
		...
		case VIODEV_MSG_IO_READ:
			 ...
			 * async channel the assert uses (virtio_assert_irq),
			 * never piggybacked on this synchronous reply.  This
			 * makes the VM-process event-loop thread the sole,
			 * in-order mutator of the device's IOAPIC line,
			 * eliminating an assert(async)/deassert(sync)
			 * cross-thread reorder that could clobber a coalesced
			 * level completion.
			 */
			if (deassert)
