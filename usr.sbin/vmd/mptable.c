 ...
 * fw_cfg "BIOS linker/loader" (etc/table-loader), the same mechanism the ACPI
 * variant of this table generator uses for the MADT.  Writing the table
 * straight to guest RAM is unreliable under SeaBIOS (it owns the EBDA / MP
 * scan regions, and its own MP table mis-resolves at high vcpu counts).
 * SeaBIOS ALLOCATEs the config in HIGH memory and the floating pointer in the
 * FSEG (scanned by the guest), ADD_POINTERs the fp->config link, and
 * ADD_CHECKSUMs both.  Needs firmware with CONFIG_FW_ROMFILE_LOAD=y and
 * CONFIG_MPTABLE off.
 */
