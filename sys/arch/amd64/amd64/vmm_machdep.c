...
vcpu_reset_regs_vmx(struct vcpu *vcpu, struct vcpu_reg_state *vrs)
{
	...
	if ((~cr4 & want0) != want0) {
		DPRINTF("%s: CR4 check 2 failed; cr4 0x%08x want0 0x%08x\n",
			__func__, cr4, want0);
		ret = EINVAL;
		goto exit;
	}
...
svm_handle_hlt(struct vcpu *vcpu)
{
	...
	if (!svm_get_iflag(vcpu, rflags)) {
		/* SMP: AP parks via cli;hlt after SeaBIOS check-in. */
		return (EAGAIN);
	}
...
vmx_handle_rdmsr(struct vcpu *vcpu)
{
	...
	case MSR_CR_PAT:
		...
		*rdx = (vcpu->vc_shadow_pat >> 32);
		break;
	case MSR_APICBASE: {
		uint64_t apicbase = 0xfee00000ULL | APICBASE_GLOBAL_ENABLE;
		if (vcpu->vc_id == 0)
...
svm_handle_msr(struct vcpu *vcpu)
{
	...
	if (vmcb->v_exitinfo1 == 1) {
		...
		case MSR_APICBASE: {
			uint64_t apicbase =
			    0xfee00000ULL | APICBASE_GLOBAL_ENABLE;
			if (vcpu->vc_id == 0)
				apicbase |= APICBASE_BSP;
			*rax = apicbase & 0xFFFFFFFFULL;
			*rdx = apicbase >> 32;
