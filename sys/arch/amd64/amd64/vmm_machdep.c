...
#ifdef MULTIPROCESSOR
	/*
	 * vc_intr is published here lock-free (the run loop holds vc_lock for
	 * the whole guest run, so blocking on it would serialize IRQ delivery
	 * behind guest execution).  This store and the vc_curcpu load below
	 * form a store-buffer (Dekker) pattern against the run loop, which
	 * publishes vc_curcpu (in vm_run) and then reads vc_intr.  On x86-TSO
	 * the store-then-load can be observed reordered, so without a full
	 * barrier we could read vc_curcpu==NULL (skip the kick) while the run
	 * loop reads a stale vc_intr==0 (never arms the interrupt window) ->
	 * the pending IRQ is stranded and the vcpu HLTs forever.
	 * membar_producer/consumer are compiler-only no-ops on amd64; the
	 * StoreLoad hazard requires membar_sync (mfence) on BOTH sides
	 * (matching barrier in vm_run after WRITE_ONCE(vc_curcpu)).
	 */
	membar_sync();
...
vcpu_run_vmx(struct vcpu *vcpu, struct vm_run_params *vrp)
{
	 ...
	 * (vc_irqready == 0), arm interrupt-window exiting so a VMEXIT is
	 * forced the instant the guest re-enables interrupts (IF 0->1), at
	 * which point vmd retries injection.  Without this, a guest spinning
	 * with interrupts disabled while waiting on a cross-CPU event never
	 * exits on its own, so a pending cross-CPU IPI (delivered to the
	 * LAPIC IRR via the PV-IPI VMCALL or a directed LAPIC IPI) is
	 * stranded forever and the waiting CPU soft-locks.  This is checked
	 * on entry as well as on exit (below), since a guest that takes no
	 * exits of its own while spinning with IF=0 would otherwise never
	 * reach the exit-side check.  When nothing is pending, clear the
	 * (otherwise wasteful) window-exiting control.
	 */
	if (vmread(VMCS_PROCBASED_CTLS, &procbased)) {
		printf("%s: can't read procbased ctls on entry\n", __func__);
		return (EINVAL);
	}
	vc_intr = READ_ONCE(vcpu->vc_intr);
	if (vc_intr && vcpu->vc_irqready == 0)
